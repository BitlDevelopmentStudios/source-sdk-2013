...
#ifdef TF_DLL
ConVar ai_generate_nodes_main_spawn("ai_generate_nodes_main_spawn", "info_player_teamspawn", FCVAR_CHEAT);
#elif HL2MP
ConVar ai_generate_nodes_main_spawn("ai_generate_nodes_main_spawn", "info_player_deathmatch", FCVAR_CHEAT);
#else
ConVar ai_generate_nodes_main_spawn("ai_generate_nodes_main_spawn", "info_player_start", FCVAR_CHEAT); //??????
#endif
...
void CNodesGeneration::Start()
{
	...
	if (m_iNavAreaCount == 0)
	{
		...
		pSpotFinder = gEntList.FindEntityByClassname(NULL, ai_generate_nodes_main_spawn.GetString());
